...
export function SabiBioPreview({ config }: { config: PreviewConfig }) {
  ...
  return (
    <div className="mx-auto w-72.5 overflow-hidden rounded-[2.2rem] border-[7px] border-zinc-800 bg-black shadow-2xl shadow-black/40">
      <div className="h-145 overflow-y-auto" style={{ background: branding.background_color || template.background, color: '#f8fafc', fontFamily }}>
        <div className="flex items-center justify-between px-4 py-3 text-[9px] text-white/50">
          <span>9:41</span><span className="h-1.5 w-16 rounded-full bg-white/10" /><span>â— â—’</span>
        </div>
        <div className="px-4 pb-8 pt-6 text-center">
          ...
          <p className="mx-auto mt-2 max-w-55 text-[10px] leading-4 text-white/60">{branding.bio || 'Your short business bio will appear here.'}</p>
          ...
          <div className="mt-6 flex justify-center gap-3 text-white/50"><span>â—Ž</span><span>â—‰</span><span>â—Œ</span></div>
        </div>
