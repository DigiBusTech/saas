    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Manage offerings for <span className="text-indigo-400 font-medium">{workspace.name}</span>
          </p>
        </div>
        <button
      ...
      {products.length === 0 ? (
        <div className="rounded-xl bg-zinc-900/60 backdrop-blur-md border border-white/10 p-12 text-center">
          ...
          <p className="text-sm text-gray-500">No products yet. Add your first product or service.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((product, i) => (
            <motion.div
              ...
              className="group rounded-xl bg-zinc-900/60 backdrop-blur-md border border-white/10
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] text-gray-500 uppercase">{product.currency}</span>
                  </div>
                  ...
                  {product.payment_link && (
                    <a
                      ...
                      className="text-[9px] text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition"
                    >
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-gray-500 mb-1.5">Product Name</label>
                  ...
                  <p className="text-[10px] text-gray-600 mt-1">The display name your customers will see when the AI shares this product.</p>
                </div>

                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-gray-500 mb-1.5">Description</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-600 mt-1">Enter the price in your selected currency. Ensure this matches your Stripe/Flutterwave configuration.</p>
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider text-gray-500 mb-1.5">Currency</label>
                    <select name="currency" defaultValue={editingProduct?.currency ?? 'USD'}
                      ...
                      <option value="NGN" className="bg-zinc-900">NGN</option>
                      <option value="EUR" className="bg-zinc-900">EUR</option>
                      <option value="GBP" className="bg-zinc-900">GBP</option>
                    </select>
                    <p className="text-[10px] text-gray-600 mt-1">Must match the currency set in your payment gateway.</p>
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-gray-500 mb-1.5">Image URL</label>
                  <input name="image_url" defaultValue={editingProduct?.image_url ?? ''} placeholder="https://example.com/product.jpg"
                    className="w-full px-3 py-2.5 rounded-lg bg-zinc-800/50 border border-white/10 text-sm text-white placeholder:text-gray-600 focus:border-indigo-500/40 outline-none transition" />
                  <p className="text-[10px] text-gray-600 mt-1">A direct URL to the product image. The AI sends this as a rich-media card to customers.</p>
                </div>

                <div>
                  <label className="block text-[10px] uppercase tracking-wider text-gray-500 mb-1.5">Payment Link</label>
